...
plt.rcParams['axes.prop_cycle'] = plt.cycler(color=color_list)
plt.plot(df["Temperature"][settings['predict_forcast_startfrom']:],label='Temp History')
plt.plot(new_data['Predictions'][:X_FUTURE_GRAPH],'--b', label='Temp prediction')
plt.grid()
axes = plt.gca()
axes.set_ylim([settings['plot_ylimit_temp_low'],settings['plot_ylimit_temp_high']])
plt.fill_between(df.index[settings['predict_forcast_startfrom']:], df["Temperature"][settings['predict_forcast_startfrom']:], alpha=.5)
